...
chapter4topline<-LatticePlotWrapper_csis360(
  ...
  geom_vline(xintercept=2006.5)+get_caption()
)
...
ggsave600dpi("Output//CompOverall.png", CompOverall, width=4.5, height= 8, units="in",size=12)
ggsave("Output//CompOverall.png", CompOverall, width=4.5, height= 8, units="in" , dpi=600)
...
ggsave600dpi("Output//UCAoverall.png", UCAoverall, width=4.5, height= 5, units="in")
...
UCAannual<-build_plot(
  ...
  theme(legend.position = "bottom")+
    ...
    scale_y_continuous(labels = scales::percent)+coord_cartesian(ylim=c(0,0.3))+
  theme(strip.text.y = element_text(angle=270))
)+facet_grid(DecisionTreeDisplay ~ NoCompOffr)
...
geom.text.size<-3
ReachbackUCA<-  LatticePlotWrapper_csis360(
  VAR.color.legend.label="Dataset",
  VAR.main.label=NULL,
  VAR.X.label="Fiscal Year",
  VAR.Y.label="Percent of Obligations",
  ...
  facet_grid(primary ~ secondary)
    # )
ReachbackUCA
ggsave600dpi("Output//ReachbackUCA.png", ReachbackUCA, width=4.5, height= 4.5, units="in")
...
UCAoverall<-  LatticePlotWrapper_csis360(
  ...
  facet_grid(primary ~ .)
    # )
UCAoverall
ggsave600dpi("Output//UCAoverall.png", UCAoverall, width=6.5, height= 5, units="in")
...
build_plot(
  ...
  labels_and_colors=labels_and_colors,
  data=subset(CompetitionReachback,!is.na(Reachback)),
  # NA, #VAR.ncol
  x_var="dFYear", #x_var
  y_var="Obligation.2017", #y_var
  color_var="NoCompOffr", #color_var
  facet_var="DecisionTreeDisplay", #facet_var
  VAR.facet.secondary="Reachback" # VAR.facet.secondary=NA
  # ,MovingAverage=0
  ...
  title="Defense Contracts with Labeled Reachback",
  xlab="Fiscal Year",
  ylab="Percent of Obligations")+theme(legend.position = "bottom")+
    scale_x_date("Fiscal Year",labels = date_format("'%y"))+
  theme(strip.text.y = element_text(angle=270))
...
UCAReachback<-ddply(UCAReachback,
                ...
                plyr::mutate,
                pObligation=Obligation.2017/sum(Obligation.2017)
                )


build_plot(
  labels_and_colors=labels_and_colors,
  data=subset(UCAReachback,!is.na(Reachback)),
  # NA, #VAR.ncol
  x_var="dFYear", #x_var
  y_var="Obligation.2017", #y_var
  color_var="isUCA", #color_var
  ...
  title="Defense Contracts with Labeled Reachback",
  xlab="Fiscal Year",
  yab="Percent of Obligations")+theme(legend.position = "bottom")+
    scale_x_date("Fiscal Year",labels = date_format("'%y"))+
  theme(strip.text.y = element_text(angle=270))
