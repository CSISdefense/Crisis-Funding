...
full_data<-full_data[,!colnames(full_data) %in% c("SubCustomer")]
debug(prepare_labels_and_colors)
labels_and_colors<-prepare_labels_and_colors(full_data,na_replaced=TRUE)
...
chapter4topline<-build_plot(
  data=subset(summary_data,!is.na(International)),
  chart_geom = "Line Chart",
  share = FALSE,
  x_var="Fiscal.Year",
  y_var="Obligation.2016",
  color_var="Is.Defense",
  facet_var="CrisisDisplay",        # name of facet variable, as string
  legend=TRUE, #Include a legend
  caption=TRUE, #Include a source caption
  labels_and_colors=Coloration,
  column_key=NULL
)
...
```
### Federal-Wide 21B: NoComOffr2 and OffCri for terminations
```{r Model21B}
# summary(large_crisis_smp$ServCommCons)
# summary(large_crisis_smp$CPSA)
...
summary(crisis_all[crisis_all$Crisis=="ARRA",])
#OffPlace is missing!!

stargazer::stargazer(CBre_21A,CBre_22A,Term_21A,Term_22A,type="text",
                       digits=2)


texreg::htmlreg(list(CBre_21A,CBre_22A,CBre_22D,Term_21B,Term_22E),file="Output//Both_Model.html",
                single.row = TRUE,
                ...
                groups = list("Study Variables" = 2:8,
                              "Crisis Category" = 9:11,
                              "Contract Characteristics"=12:22,
"Interactions" = 23:26),
                custom.coef.map=list("(Intercept)"="(Intercept)",
                                     "NoCompOffrUrgency"="Urgency Waiver",
...
"NoCompOffrUrgency:cl_Ceil"="Urgency Waiver:Log(Init. Ceiling)",
...
"NoCompOffrUrgency:b_UCA"="Urgency Waiver:UCA",
