...
CBre_02F <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO +cn_Offr+
                   cn_Offr:ARRA + cn_Offr:Dis + cn_Offr:OCO, family=binomial(link="logit"))
display(CBre_02F)
...
CBre_02G_plot<-ggplot(data=fed_smp,
...
#Plot the data and fitted curve
CBre_02H_plot<-ggplot(data=fed_smp,
       aes(y=j_Term,x=cl_Offr))+geom_jitter(alpha=0.11,height=0)+scale_y_sqrt() +
  labs(title="Fitted Linear Model", caption="Source: FPDS, CSIS Analysis")
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_02H)
...
CBre_02B_curve<-function(x, Comp){invlogit(cbind(1,Comp,x) %*%  coef(CBre_02B))}
...
#Plot the fitted model plot
CBre_02D_curve<-function(x, Comp){invlogit(cbind(1,Comp,x, x^2) %*%  coef(CBre_02D))}

#Competition curves
fitted_cbre_model(fed_smp,"cl_Ceil") + stat_function(fun = CBre_02D_curve, 
                             args=list(Comp=0),color="blue")+
...
residuals_cbre_plot(CBre_02D,"cl_Ceil")+
  labs(x="Centered Log(Ceiling)")
...
CBre_02E_curve<-function(x, Comp){invlogit(cbind(1,Comp,x,x^2,Comp*x) %*%  coef(CBre_02E))}

#Competition Curves
fitted_cbre_model(fed_smp,"cl_Ceil") + stat_function(fun = CBre_02E_curve, 
                             args=list(Comp=0),color="blue")+
...
residuals_cbre_plot(CBre_02E,"cl_Ceil")+
  labs(x="Centered Log(Ceiling)")
```
...
CBre_02G_curve<-function(x, Comp,Ceil){invlogit(cbind(1,Comp,Ceil,Ceil^2,x,Comp*Ceil) %*%  coef(CBre_02G))}

#Competition Curves
fitted_cbre_model(fed_smp,"cl_Days") + stat_function(fun = CBre_02G_curve, 
...
residuals_cbre_plot(CBre_02G)+
  labs(x="Estimated  Pr (Termination)")
# debug(binned.resids)
# residuals_cbre_plot(CBre_02G,"cl_Days")+
  # labs(x="Centered Log(Ceiling)")
