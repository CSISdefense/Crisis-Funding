...
NA_stats(fed,"n_Comp")' 
* n_Offr is a numeric variable based on competition and with more granularity on number offers. It has a value of 0 for uncompeted contracts and a value of 1 for single-offer competition, and a value of 2 for 2-offers, 3 for 3-4 offers, and 4 for 5+ offers. 'r summary
(fed$n_Offr);
NA_stats(fed,"n_Offr")' *****PROBLEM******
...
# summary(fed_smp$b_Comp )
# summary(fed_smp$n_Comp )
# summary(fed_smp$Comp )
# summary(fed_smp$CompOffr )
# summary(fed_smp$EffComp )

complete<-!is.na(fed$b_Term)&
  !is.na(fed$b_CBre)&
  (!is.na(fed$b_Comp) | !is.na(fed$l_Offr))&
  !is.na(fed$l_Ceil)&
  !is.na(fed$l_Days)&
  !is.na(fed$Veh) &
  !is.na(fed$n_Fixed)&
...
sum(fed[!complete,]$Action.Obligation,na.rm=TRUE)/sum(fed$Action.Obligation,na.rm=TRUE)
...
```{r Model01A}
levels(fed_smp$MaxOfDecisionTree)[levels(fed_smp$MaxOfDecisionTree)=="Excluded"]<-
  "None"
fed_smp$MaxOfDecisionTree[is.na(fed_smp$MaxOfDecisionTree)]<-"None"
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"MaxOfDecisionTree")

#Percent Terminated Plot
discrete_percent_cbre_plot(fed_smp,"MaxOfDecisionTree")

#Model
CBre_01A <- glm (data=fed_smp,
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"Comp")

#Percent Terminated Plot
discrete_percent_cbre_plot(fed_smp,"Comp")

#Model
CBre_02A <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO +cb_Comp, family=binomial(link="logit"))
display(CBre_02A)

#Plot the new variable and coefficients
#Add the fitted regression line
discrete_fitted_cbre_model(fed_smp,"cb_Comp") + 
  stat_function(fun = fit_curve, args=list(a=coef(CBre_02A)[1],                                                        b=coef(CBre_02A)[2]),color="blue")



```
Competition is associated with a significantly higher rate of terminations. This is surprising based on the literature.The next step is to check alternate formulations. There's not yet enough in this model for any meaningful residual models. The next step is to look at a more detailed model, where single offer competition is broken out as a middle option between competition and no competion.

###Model 02B: No Competition / Single Offer / Competition
```{r Model02B}

#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"EffComp")


#Percent Terminated Plot
discrete_percent_cbre_plot(fed_smp,"EffComp")

#Model
CBre_02B <- glm (data=fed_smp,
...
#Plot the Fitted
#Add the fitted regression line
# discrete_fitted_cbre_model(fed_smp,"n_Comp") +
#   stat_function(fun = fit_curve, args=list(a=coef(CBre_02B)[1],
#                                            b=coef(CBre_02B)[2]),
#                 color="blue")
...
###Model 02C: No Competition / Number of Offer Categories 
```{r Model02C}
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"CompOffr")

#Percent Terminated Plot
discrete_percent_cbre_plot(fed_smp,"CompOffr")

#Model
CBre_02C <- glm (data=fed_smp,
...
###Model 02D: Number of Offer Categories 
```{r Model02D}

#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"l_Offr")

#Percent Terminated Plot 
binned_percent_cbre_plot(fed_smp,"l_Offr")

#Percent Terminated Plot
binned_percent_cbre_plot(fed_smp,"l_Offr","FxCb")

#Percent Terminated Plot
binned_percent_cbre_plot(fed_smp,"l_Offr","PSR")

#Percent Terminated Plot
binned_percent_cbre_plot(fed_smp,"l_Offr","Veh")


#Model
CBre_02D <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO +cl_Offr, family=binomial(link="logit"))
display(CBre_02D)

#Variant Model
fed_smp$clsqr_Offer<-fed_smp$cl_Offr^2
CBre_11E <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO +cl_Offr +clsqr_Offer, family=binomial(link="logit"))
display(CBre_11E)

#Plot the data and fitted curve
CBre_02D_plot<-ggplot(data=fed_smp,
...
#Plot the Fitted
#Add the fitted regression line
discrete_fitted_cbre_model(fed_smp,"cl_Offr") +
  stat_function(fun = fit_curve, args=list(a=coef(CBre_02D)[1],
                                           b=coef(CBre_02D)[2]),
                             color="blue")

#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_02D)



#Plot residuals versus fitted
residuals_cbre_plot(CBre_02D,bins=3)+
...
freq_continuous_cbre_plot(fed_smp,"UnmodifiedContractBaseAndAllOptionsValue",
               bins=1000)
freq_continuous_cbre_plot(subset(fed_smp,UnmodifiedContractBaseAndAllOptionsValue<100000000),
               "UnmodifiedContractBaseAndAllOptionsValue",
               bins=1000)

#Frequency Plot for logged ceiling
freq_continuous_cbre_plot(fed_smp,"l_Ceil")

#Percent Terminated Plot
binned_percent_cbre_plot(fed_smp,"l_Ceil")

#Model
CBre_02A <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + cl_Ceil, family=binomial(link="logit"))
display(CBre_02A)
...
#Competition curves
fitted_cbre_model(fed_smp,"cl_Ceil") + stat_function(fun = CBre_02A_curve, 
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_02A)

#Plot residuals versus fitted
residuals_cbre_plot(CBre_02A)+
  labs(x="Estimated  Pr (Termination)")

residuals_cbre_plot(CBre_02A,"cl_Ceil")+
...
###Model 12B: Cost Ceiling Squared
...
#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"lsqr_Ceil")

#Percent Terminated Plot
binned_percent_cbre_plot(fed_smp,"lsqr_Ceil")
...
#Plot the fitted model plot
CBre_02B_curve<-function(x, Comp){invlogit(cbind(1,Comp,x, x^2) %*%  coef(CBre_02B))}

#Competition curves
fitted_cbre_model(fed_smp,"cl_Ceil") + stat_function(fun = CBre_02B_curve, 
                             args=list(Comp=0),color="blue")+
  stat_function(fun = CBre_02B_curve, 
                             args=list(Comp=2),color="blue")

#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_02B)

#Plot residuals versus fitted
residuals_cbre_plot(CBre_02B)+
  labs(x="Estimated  Pr (Termination)")

residuals_cbre_plot(CBre_02B,"cl_Ceil")+
  labs(x="Centered Log(Ceiling)")
...
###Model 12C: Cost Ceiling and Competition

```{r Model12C}

#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"l_Ceil","EffComp")

#Plot the percent terminated across cost ceiling and competition
binned_percent_cbre_plot(fed_smp,"l_Ceil","EffComp")
...
CBre_02C_curve<-function(x, Comp){invlogit(cbind(1,Comp,x,x^2,Comp*x) %*%  coef(CBre_02C))}

#Competition Curves
fitted_cbre_model(fed_smp,"cl_Ceil") + stat_function(fun = CBre_02C_curve, 
                             args=list(Comp=0),color="blue")+
  stat_function(fun = CBre_02C_curve, 
                             args=list(Comp=2),color="blue")

#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_02C)

#Plot residuals versus fitted
residuals_cbre_plot(CBre_02C)+
  labs(x="Estimated  Pr (Termination)")

residuals_cbre_plot(CBre_02C,"cl_Ceil")+
  labs(x="Centered Log(Ceiling)")
```
...
###Model 12D: Maximum Duration

```{r Model12D}
#Frequency Plot for max duration
freq_continuous_cbre_plot(fed_smp,"UnmodifiedDays",
               bins=1000)

#Frequency Plot for logged max duration
freq_continuous_cbre_plot(fed_smp,"l_Days")

#Percent Terminated Plot
binned_percent_cbre_plot(fed_smp,"l_Days")
...
CBre_02D_curve<-function(x, Comp,Ceil){invlogit(cbind(1,Comp,Ceil,Ceil^2,x,Comp*Ceil) %*%  coef(CBre_02D))}

#Competition Curves
fitted_cbre_model(fed_smp,"cl_Days") + stat_function(fun = CBre_02D_curve, 
                             args=list(Comp=0,Ceil=0),color="blue")+
  stat_function(fun = CBre_02D_curve, 
...
#Plot residuals versus fitted
residuals_cbre_plot(CBre_02D)+
  labs(x="Estimated  Pr (Termination)")
# debug(binned.resids)
# residuals_cbre_plot(CBre_02D,"cl_Days")+
  # labs(x="Centered Log(Ceiling)")
...
###Model 12E: Interation of Ceiling and Duration

```{r Mode02E}

#First a quick scatter plot for terminations by duration and ceiling
ggplot(data=subset(fed_smp,!is.na(l_Ceil)),
...
#Create the model
CBre_02E <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + clsqr_Ceil + cl_Days+
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days, family=binomial(link="logit"))
display(CBre_02E)

CBre_02E_curve<-function(x, Comp,Ceil){invlogit(
  cbind(1,Comp,Ceil,Ceil^2,x,
        Comp*Ceil,x*Ceil) %*%  coef(CBre_02E))}



#Competition Curves
fitted_cbre_model(fed_smp,"cl_Days") + stat_function(fun = CBre_02E_curve, 
                             args=list(Comp=0,Ceil=0),color="blue")+
  stat_function(fun = CBre_02E_curve, 
                             args=list(Comp=2,Ceil=0),color="blue")
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_02E)


#Plot residuals versus fitted
residuals_cbre_plot(CBre_02E)+
  labs(x="Estimated  Pr (Termination)")
# debug(binned.resids)
# residuals_cbre_plot(CBre_02E,"cl_Days")+
  # labs(x="Centered Log(Ceiling)")
...
#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"lsqr_Days")

#Percent Terminated Plot
binned_percent_cbre_plot(fed_smp,"lsqr_Days")


#Create the model
CBre_02F <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + clsqr_Ceil + cl_Days+ clsqr_Days +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days, family=binomial(link="logit"))
display(CBre_02F)

CBre_02F_curve<-function(x, Comp,Ceil){invlogit(
  cbind(1,Comp,Ceil,Ceil^2,x,x^2,
        Comp*Ceil,x*Ceil) %*%  coef(CBre_02F))}

#Competition curves
fitted_cbre_model(fed_smp,"cl_Ceil") + stat_function(fun = CBre_02F_curve, 
                             args=list(Comp=0),color="blue")+
  stat_function(fun = CBre_02F_curve, 
                             args=list(Comp=2),color="blue")

#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_02F)

#Plot residuals versus fitted
residuals_cbre_plot(CBre_02F)+
  labs(x="Estimated  Pr (Termination)")
...
residuals_cbre_plot(CBre_02F,"cl_Ceil")+
  labs(x="Centered Log(Ceiling)")
...
###Model 12G: Testing the removal of Ceiling^2 

```{r Model12G}

#Create the model
CBre_02G <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ clsqr_Days +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days, family=binomial(link="logit"))
display(CBre_02G)

CBre_02G_curve<-function(x, Comp,Ceil){invlogit(
  cbind(1,Comp,Ceil,x,x^2,
        Comp*Ceil,x*Ceil) %*%  coef(CBre_02E))}

#Competition curves
fitted_cbre_model(fed_smp,"cl_Ceil") + stat_function(fun = CBre_02G_curve, 
                             args=list(Comp=0),color="blue")+
  stat_function(fun = CBre_02G_curve, 
                             args=list(Comp=2),color="blue")

#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_02G)

#Plot residuals versus fitted
residuals_cbre_plot(CBre_02G)+
  labs(x="Estimated  Pr (Termination)")

residuals_cbre_plot(CBre_02G,"cl_Ceil")+
  labs(x="Centered Log(Ceiling)")
...
###Model 12H: Interaction of Duration and Competition
```{r Mode02H}

#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"l_Days","EffComp")

#Plot the percent terminated across cost ceiling and competition
binned_percent_cbre_plot(fed_smp,"l_Days","EffComp")

#Create the model
CBre_02H <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + clsqr_Ceil + cl_Days+ clsqr_Days +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days, family=binomial(link="logit"))
display(CBre_02H)

CBre_02H_curve<-function(x, Comp,Ceil){invlogit(
  cbind(1,Comp,Ceil,Ceil^2,x,x^2,
        Comp*Ceil,x*Ceil,x*Comp) %*%  coef(CBre_02H))}

#Competition curves
fitted_cbre_model(fed_smp,"cl_Days") + stat_function(fun = CBre_02H_curve, 
                             args=list(Comp=0,Ceil=0),color="blue")+
  stat_function(fun = CBre_02H_curve, 
                             args=list(Comp=2,Ceil=0),color="blue")
...
```{r Model13A}
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"Veh")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"Veh")


#Create the model
CBre_03A <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + clsqr_Ceil + cl_Days+ clsqr_Days +
...
#Competition curves
discrete_fitted_cbre_model(fed_smp,"SIDV") +
...
#Competition curves
discrete_fitted_cbre_model(fed_smp,"MIDV") +
...
#Competition curves
discrete_fitted_cbre_model(fed_smp,"OIDV") +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_03A)
...
#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"l_Days","Veh")


#Percent Terminated Plot 
binned_percent_cbre_plot(fed_smp,"l_Days","Veh")

#Create the model
CBre_03B <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + clsqr_Ceil + cl_Days+ clsqr_Days +
                   SIDV + MIDV + FSSGWAC + BPABOA +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_03B)
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"EffComp","Veh")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"EffComp","Veh")


#Create the model
CBre_03C <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + clsqr_Ceil + cl_Days+ clsqr_Days +
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_03C)
...
#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"l_Ceil","Veh")


#Percent Terminated Plot 
binned_percent_cbre_plot(fed_smp,"l_Ceil","Veh")

#Create the model

#Create the model
CBre_03D <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + clsqr_Ceil + cl_Days+ clsqr_Days +
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_03D)
...
#Competition curves
discrete_fitted_cbre_model(fed_smp,"cl_Ceil") +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_03E)
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_03F)
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"FxCb")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"FxCb")


#Create the model
CBre_04A <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
CBre_04A_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV){invlogit(
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_04A)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_04A)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"l_Days","FxCb")

#Percent Terminated Plot 
binned_percent_cbre_plot(fed_smp,"l_Days","FxCb")


#Create the model
CBre_04B <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
# Plot the model and Vehicle 
CBre_04B_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        x,
        Days*x,Days*MIDV,Days*OIDV,
        Comp*x,Comp*MIDV,Comp*OIDV,
        Days*x) %*%  coef(CBre_04B))}
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_04B)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_04B)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"FxCb","Veh")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"FxCb","Veh")

#Create the model
CBre_04C <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Fixed +
...
# Plot the model and Vehicle for SIDV
CBre_04C_curve<-function(x, Comp,Ceil,Days,MIDV,OIDV){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        n_Fixed,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Days*SIDV,Days*MIDV,Days*OIDV,
...
#Vehicle curves
discrete_fitted_cbre_model(fed_smp,"n_Fixed") +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_04C)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_04C)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"Fee")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"Fee")



#Create the model
CBre_04D <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
...
#Days curves
discrete_fitted_cbre_model(fed_smp,"n_Incent") +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_04D)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_04D)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"Fee","FxCb")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"Fee","FxCb")+
   theme(axis.text.x = element_text(angle = 90, hjust = 1))



#Create the model
CBre_04E <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
# Plot the model and Vehicle 
CBre_04E_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,x,
        Days*x,Days*MIDV,Days*OIDV,
        Comp*x,Comp*MIDV,Comp*OIDV,
        x*Fixed) %*%  coef(CBre_04E))}

#Days curves
discrete_fitted_cbre_model(fed_smp,"SIDV") +
  stat_function(fun = CBre_04E_curve,
                             args=list(Comp=0,Ceil=0,Days=0,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_04E)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_04E)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"UCA")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"UCA")



#Create the model
CBre_04F <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
# Plot the model and Vehicle 
CBre_04F_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,x,
        Days*x,Days*MIDV,Days*OIDV,
        Comp*x,Comp*MIDV,Comp*OIDV) %*%  coef(CBre_04F))}

#Days curves
discrete_fitted_cbre_model(fed_smp,"SIDV") +
  stat_function(fun = CBre_04F_curve,
                             args=list(Comp=0,Ceil=0,Days=0,
                                       MIDV=0,SIDV=0,OIDV=0,
                                       Fixed=1),color="blue")+
  stat_function(fun = CBre_04F_curve,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_04F)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_04F)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"UCA","n_Comp")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"UCA","n_Comp")



#Create the model
CBre_04G <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
# Plot the model and Vehicle 
CBre_04G_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,x,
        Days*x,Days*MIDV,Days*OIDV,
        Comp*x,Comp*MIDV,Comp*OIDV) %*%  coef(CBre_04G))}

#Days curves
discrete_fitted_cbre_model(fed_smp,"SIDV") +
  stat_function(fun = CBre_04G_curve,
                             args=list(Comp=0,Ceil=0,Days=0,
                                       MIDV=0,SIDV=0,OIDV=0,
                                       Fixed=1),color="blue")+
  stat_function(fun = CBre_04G_curve,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_04G)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_04G)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_continuous_cbre_plot(fed_smp,"l_Days","UCA")

#Percent Terminated Plot 
binned_percent_cbre_plot(fed_smp,"l_Days","UCA")


#Create the model
CBre_04H <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_04H)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_04H)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"Intl")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"Intl")



#Create the model
CBre_05A <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
# Plot the model and Vehicle 
CBre_05A_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,x,
        Days*x,Days*MIDV,Days*OIDV,
        Comp*x,Comp*MIDV,Comp*OIDV,
        UCA*Comp) %*%  coef(CBre_05A))}

#Days curves
discrete_fitted_cbre_model(fed_smp,"SIDV") +
  stat_function(fun = CBre_05A_curve,
                             args=list(Comp=0,Ceil=0,Days=0,
                                       MIDV=0,SIDV=0,OIDV=0,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_05A)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_05A)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"Intl","FxCb")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"Intl","FxCb")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"Intl","Veh")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"Intl","Veh")



#Create the model
CBre_05C <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
# Plot the model and Vehicle 
CBre_05C_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,x,
        Days*x,Days*MIDV,Days*OIDV,
        Comp*x,Comp*MIDV,Comp*OIDV,
        UCA*Comp) %*%  coef(CBre_05C))}
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_05C)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_05C)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"Who")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))


#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"Who")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))


fed_smp$b_ODoD<-fed_smp$Who
levels(fed_smp$b_ODoD)<- list("1"=c("Other DoD"), 
                                "0"=c("Air Force","Army","Navy"))
fed_smp$b_ODoD[fed_smp$b_ODoD=="Uncategorized"]<-NA
fed_smp$b_ODoD<-as.integer(as.character(fed_smp$b_ODoD))

#Create the model
CBre_06A <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
# Plot the model and Vehicle 
CBre_06A_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA,Intl){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,Intl,x,
        Days*x,Days*MIDV,Days*OIDV,
        Comp*x,Comp*MIDV,Comp*OIDV,
        UCA*Comp) %*%  coef(CBre_06A))}

#Days curves
discrete_fitted_cbre_model(fed_smp,"b_ODoD") +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_06A)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_06A)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"EffComp","Who")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))


#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"EffComp","Who")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
# Plot the model and Vehicle 
CBre_06B_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA,Intl){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,Intl,x,
        Days*x,Days*MIDV,Days*OIDV,
        Comp*x,Comp*MIDV,Comp*OIDV,
        UCA*Comp) %*%  coef(CBre_06B))}
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_06B)
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"PSR")

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"PSR")



#Create the model
CBre_08A <- glmer(data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  + cl_Ceil:cl_Days  + n_Comp:cl_Days +
...
# Plot the model and Vehicle 
CBre_08A_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,x,
        Days*x,Days*MIDV,Days*OIDV,
        UCA*Comp) %*%  coef(CBre_08A))}

#Days curves
discrete_fitted_cbre_model(fed_smp,"SIDV") +
  stat_function(fun = CBre_08A_curve,
                             args=list(Comp=0,Ceil=0,Days=0,
                                       MIDV=0,SIDV=0,OIDV=0,
                                       Fixed=1,UCA=0),color="blue")+
  stat_function(fun = CBre_08A_curve,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_08A)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_08A)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"PSR_What")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"PSR_What")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))



#Create the model
CBre_08B <- glmer(data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  +
...
# Plot the model and Vehicle 
CBre_08B_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,x,
        Days*x,Days*MIDV,Days*OIDV,
        UCA*Comp) %*%  coef(CBre_08B))}

#Days curves
discrete_fitted_cbre_model(fed_smp,"SIDV") +
  stat_function(fun = CBre_08B_curve,
                             args=list(Comp=0,Ceil=0,Days=0,
                                       MIDV=0,SIDV=0,OIDV=0,
                                       Fixed=1,UCA=0),color="blue")+
  stat_function(fun = CBre_08B_curve,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_08B)

#Plot residuals versus fittedO
residuals_cbre_plot(CBre_08B)+
  labs(x="Estimated  Pr (Termination)")
...
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"PSR_What","EffComp")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"EffComp","PSR")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

discrete_percent_cbre_plot(fed_smp,"EffComp","What")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

discrete_percent_cbre_plot(fed_smp,"EffComp","PSR_What")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))


#Effective Competition
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"PSR_What","Veh")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"Veh","PSR")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

discrete_percent_cbre_plot(fed_smp,"Veh","What")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

discrete_percent_cbre_plot(fed_smp,"Veh","PSR_What")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

#FxCb
#Frequency Plot
freq_discrete_cbre_plot(fed_smp,"PSR_What","FxCb")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

#Percent Terminated Plot 
discrete_percent_cbre_plot(fed_smp,"FxCb","PSR")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

discrete_percent_cbre_plot(fed_smp,"FxCb","What")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

discrete_percent_cbre_plot(fed_smp,"FxCb","PSR_What")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))


CBre_08C <- glmer(data=fed_smp,
                 b_CBre ~ #n_Comp + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cl_Ceil  +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_08C)


#Plot residuals versus fittedO
residuals_cbre_plot(CBre_08C)+
  labs(x="Estimated  Pr (Termination)")


# CBre_08E <- glmer(data=fed_smp,
#                  b_CBre ~ ARRA + Dis + OCO +n_Comp + 
#                    cl_Ceil + cl_Days+ 
#                    SIDV + MIDV + FSSGWAC + BPABOA +
#                    n_Comp:cl_Ceil  +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(CBre_08D)


#Plot residuals versus fittedO
residuals_cbre_plot(CBre_08D)+
  labs(x="Estimated  Pr (Termination)")
