...
# memory.limit(24566)
# 
# colnames(fed)[colnames(fed)=="n_Offr"]<-"nq_Offr"
#     # fed$q_Offr[is.na(fed$q_Offr)&
#     #              !is.na(fed$b_Comp)&
#     #              fed$b_Comp==0
#     #            ]<-"1"
#     fed$nq_Offr<-fed$q_Offr
#     levels(fed$nq_Offr) <-
#       list("1"=c("1"),
#            "2"=c("2"),
#            "3"=c("3-4"),
#            "4"=c("5+"))
#     fed$nq_Offr<-as.integer(as.character(fed$nq_Offr))
#     fed$nq_Offr[fed$b_Comp==0 & !is.na(fed$b_Comp)]<-0
#     fed$nq_Offr[is.na(fed$b_Comp)]<-NA
#     
#     fed$CompOffr<-factor(fed$nq_Offr)
#     
#     
#     
#     levels(fed$CompOffr) <-
...
#            "5+ offers"="4")
# 
#     fed$NoCompOffr<-fed$CompOffr
#     levels(fed$NoCompOffr) <-
#       list("No Competition"="No Competition",
#            "1 offer"="1 offer",
#            "2-4 offers"=c("2 offers","3-4 offers"),
#            "5+ offers"="5+ offers")
#     fed$NoCompOffr<-as.character(fed$NoCompOffr)
#     fed$NoCompOffr[is.na(fed$NoComp) |
#                             fed$NoComp!="Any Comp."]<-
#       as.character(fed$NoComp[is.na(fed$NoComp) |
#                             fed$NoComp!="Any Comp."])
#     fed$NoCompOffr<-factor(fed$NoCompOffr,c(
#       c("Other No",
#         "Urgency",
#         "1 offer",
#         "2-4 offers",
...
stargazer::stargazer(CBre_02B,CBre_02D,CBre_02E, CBre_02F,Term_02B,Term_02D,Term_02E,Term_02F,
                     type="text",
                     digits=2)
...
stargazer::stargazer(CBre_02F,CBre_02G,CBre_02H,Term_02F,Term_02G,Term_02H,
                     type="text",
                     digits=2)

summary_residual_compare(CBre_02F,CBre_02H,Term_02F,Term_02H,bins=3)
...
stargazer::stargazer(CBre_02B,CBre_02F,CBre_02I,CBre_02J,Term_02B,Term_02F,Term_02I,Term_02J,type="text",
                     digits=2)



summary_residual_compare(CBre_02B,CBre_02J,Term_02B,Term_02J,bins=3)
summary_residual_compare(CBre_02F,CBre_02J,Term_02F,Term_02J,bins=3)
...
### 02K: Urgency + Offers Interaction
```{r Model02K}
...
stargazer::stargazer(CBre_02E,CBre_02I,CBre_02K,CBre_02L,Term_02E,Term_02I,Term_02K,Term_02L,
                     type="text",
                     digits=2)

summary_residual_compare(CBre_02F,CBre_02L,Term_02F,Term_02L,bins=7)
summary_residual_compare(CBre_02J,CBre_02L,Term_02J,Term_02L,bins=3)
summary_residual_compare(CBre_02K,CBre_02L,Term_02K,Term_02L,bins=7)
...
stargazer::stargazer(CBre_02L,CBre_03A,Term_02L,Term_03A,type="text",
                     digits=2)

summary_residual_compare(CBre_02L,CBre_03A,Term_02L,Term_03A,bins=7)
...
summary_residual_compare(CBre_03A,CBre_03B,Term_03A,Term_03B,bins=7)
...
```{r Model05E}
summary_continuous_plot(crisis_smp,"l_Ceil","NoCompOffr")
#Model
...
```{r Mode06F}
summary_continuous_plot(crisis_smp,"l_Days","NoCompOffr")
...
#Update after VIF finding and removing NoCompOffr:Crisis
...
CBre_07C <- glm (data=crisis_smp,
                 b_CBre ~ Crisis +NoCompOffr +
                   ...
                   cl_Days+
                   Veh+
                  cl_Ceil:NoCompOffr+cl_Days:NoCompOffr+
...
```{r Model07D}
  ...
  summary_discrete_plot(crisis_smp,"NoCompOffr","Veh")
...
summary(crisis_smp$Term[crisis_smp$NoCompOffr=="Urgency"&crisis_smp$Veh=="M-IDC"])
...
```{r Model08F}
  summary_discrete_plot(crisis_smp,"n_Fixed","NoCompOffr")
...
```{r Model13C}
    summary_discrete_plot(crisis_smp,"NoCompOffr","Is.Defense")
...
CBre_16G<- glmer (data=large_crisis_smp,
                     ...
                     NoCompOffr:Veh + #This may be the  next to drop, though the urgency results are interested.
                     # Reach:Crisis This 
                   + Crisis:Is.Defense +
                       (1 | ServCommCons)+(1 |  CPSA) +
                       (1|ProdServ),
...
Dropped interactions: NoCompOffr:Crisis, b_UCA:Crisis, Reach:Crisis, Reach:NoCompOffr,          cl_Ceil:Veh

Added interactions: cl_Days:NoCompOffr, Crisis:Is.Defense
...
```{r Prodserv2}
  ...
  CBre_16F6<-glmer(formula = b_CBre ~ Crisis + 
                     ...
                     n_Fixed + 
                     Is.Defense + 
                     # cl_Days:NoCompOffr + We're not that close to the max|grad, this is significant but runs contrary 
                     #to expectation and is less interesting than the NoCompOffr:Veh
                     cl_Days:Veh + 
                     # cl_Ceil:Veh + Only significant relevant for BPA/BOA, runs contrary to expectation.
                     NoCompOffr:Veh + #This may be the  next to drop, though the urgency results are interested.
                     # Reach:Crisis This 
...
Removing additional variables (cl_Days:NoCompOffr, cl_Ceil:Veh) for the third and final model further increased the max|grad, not helpful.
...
# CBre_16F9<-glmer(formula = b_CBre ~ Crisis + NoCompOffr + b_UCA + Reach + c_OffCri + 
#     cl_Ceil + cl_Days + Veh + n_Fixed + Is.Defense + cl_Days:NoCompOffr + 
#     cl_Days:Veh + NoCompOffr:Veh + +Crisis:Is.Defense + (1 | ServCommCons) + 
...
summary_discrete_plot(large_crisis_smp,"Veh","NoCompOffr")
summary_continuous_plot(large_crisis_smp,"l_Days","Veh")
table(fed$Crisis,fed$NoCompOffr,fed$Veh)
table(fed$Crisis,fed$NoCompOffr,fed$Veh,fed$CBre)
table(fed$Crisis,fed$NoCompOffr,fed$Veh,fed$CBre)
...
```
The next to test removing is top drop NoCompOffr:Veh which often did not have significant differences between the no comp other and urgency variants, although it did in some cases. That dramatically ups the max|grad| to 0.01, so NoCompOffr:Veh will be staying in.
...
```
Removing NoCompOffr:Veh still ups the max|grid, in this case to 0.005.
...
Dropping cl_Days:NoCompOffr because of the comparatively small effect size.
...
Let's try dropping NoCompOffr:Veh because it's only really interestingly significant for FSS/GWAC
