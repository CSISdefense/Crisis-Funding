...
complete<-!is.na(crisis$b_Term)&
  !is.na(crisis$b_CBre)&
  (!is.na(crisis$n_Comp)|!is.na(crisis$b_Comp))&
  !is.na(crisis$l_Ceil)&
  !is.na(crisis$l_Days)&
  !is.na(crisis$Veh) &
...
freq_discrete_term_plot(crisis_smp,"Comp","MaxOfDecisionTree")
...
```
The implications of adding urgency are fairly complex. Unfortunately, some of the sign switches and changes in magnitude may be driven by the reduction in observations resulting from missing data on why a contract wasn't competed.
...
```
The interaction of OCO and urgency is perhaps most interesting, as it is significantly associated with higher rates of contract termiantion. That said, the small sample sizes result in large standard erros.  It is probably best to leave it out, or include it for OCO only.
##Scope Variables
