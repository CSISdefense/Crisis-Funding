...
crisis_with_na<-fed[fed$Crisis %in% c("ARRA","Disaster"),]
...
complete<-!is.na(crisis_with_na$b_Term)&
  ...
  !is.na(crisis_with_na$ARRA)&
!is.na(crisis_with_na$Dis)&
!is.na(crisis_with_na$OCO)&
...
stargazer::stargazer(CBre_04C,CBre_04D,CBre_04E,CBre_04F,Term_04C,Term_04D,Term_04E,Term_04F,type="text",
                     digits=2)
...
### Model 04B: Office Crisis Percent
...
stargazer::stargazer(CBre_03B,CBre_04A,CBre_04B,Term_03B,Term_04A,Term_04B,type="text",
                     digits=2)
# Plot the model and Vehicle 
CBre_04B_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
...
gridExtra::grid.arrange(residuals_cbre_plot(CBre_04B,"CrisisPercent",bins=10)+
  labs(x="Centered Log(Ceiling)"),
                        residuals_term_plot(Term_04B,"CrisisPercent",bins=10)+
  labs(x="Centered Log(Ceiling)"))
...
```
Unlike the look at location, adding the confirmed crisis percent for contracting offices does align with the reachback hypothesis, with a large and significant coefficient for both terminations and ceiling breaches. One specific phenomenon of note, for ARRA and OCO, this new term means that crisis funding itself is no longer significantly associated with higher terminations.
### Model 04C: Office Crisis % Sqrt
...
CBre_04C <- glm (data=crisis_smp,
                 b_CBre ~ Crisis +cb_Comp + b_Urg +
                   b_UCA+CrisisPercent+ sqrt_OffCri+
                   cb_Comp:Crisis+
                   b_Urg:Crisis+
                   b_UCA:Crisis , 
                 family=binomial(link="logit"))
...
#Plot residuals versus fitted
gridExtra::grid.arrange(residuals_cbre_plot(CBre_04C,bins=10)+
  labs(x="Estimated  Pr (Ceiling Breach)"),
                        residuals_term_plot(Term_04C,bins=10)+
  labs(x="Estimated  Pr (Termination)"))




gridExtra::grid.arrange(residuals_cbre_plot(CBre_04C,"CrisisPercent",bins=10)+
  labs(x="Centered Log(Ceiling)"),
                        residuals_term_plot(Term_04C,"CrisisPercent",bins=10)+
  labs(x="Centered Log(Ceiling)"))
...
```
The results are different between the two variables. For ceiling breaches, the having a moderate amount of crisis funding is associated with higher risks, but that drops off as the percent grows. For terminations, the sqrt is not significant, but shows no reduction for larger percents.

### Model 04D: Both Office
...
CBre_04D <- glm (data=crisis_smp,
                 b_CBre ~ Crisis +cb_Comp + b_Urg +
                   b_UCA+
                   AvgPlaceOCOcrisisScore+CrisisPercent+ sqrt_OffCri+
                   cb_Comp:Crisis+
                   b_Urg:Crisis+
                   b_UCA:Crisis , 
                 family=binomial(link="logit"))
...
#Plot residuals versus fitted
gridExtra::grid.arrange(residuals_cbre_plot(CBre_04D,bins=10)+
  labs(x="Estimated  Pr (Ceiling Breach)"),
                        residuals_term_plot(Term_04D,bins=10)+
  labs(x="Estimated  Pr (Termination)"))



gridExtra::grid.arrange(residuals_cbre_plot(CBre_04D,"CrisisPercent",bins=10)+
  labs(x="Centered Log(Ceiling)"),
                        residuals_term_plot(Term_04D,"CrisisPercent",bins=10)+
  labs(x="Centered Log(Ceiling)"))
...
stargazer::stargazer(CBre_03B,CBre_05A,Term_03B,Term_05A,type="text",
                     digits=2)
