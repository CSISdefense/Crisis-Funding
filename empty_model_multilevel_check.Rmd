...
if(!exists("NestedOffice_cbre"))
  NestedOffice_cbre <- glmer(data=large_crisis_smp,
                             b_CBre ~ (1 | Agency/Office)
                             , family=binomial(link="logit"))
...
if(!exists("NestedOffice_term"))
  NestedOffice_term <- glmer(data=large_crisis_smp,
                             b_Term ~ (1 | Agency/Office)
                             , family=binomial(link="logit"))
...
if(!exists("ssc_psc_office_FY_cbre"))
  ssc_psc_office_FY_cbre <- glmer(data=large_crisis_smp,
                              b_CBre ~  (1 | ServCommCons/ProdServ) + (1 | Agency:Office)+ (1|StartFY)
                              , family=binomial(link="logit"))
get_icc(display=TRUE,ssc_psc_office_FY_cbre)

if(!exists("psc_office_FY_term"))
  psc_office_FY_term <- glmer(data=large_crisis_smp,
                              b_Term ~  (1 | ProdServ) + (1 | Agency:Office) + (1|StartFY)
                              , family=binomial(link="logit"))
...
if(!exists("psc_Office_cbre"))
  psc_Office_cbre <- glmer(data=large_crisis_smp,
                             b_CBre ~  (1 | ProdServ) + (1 | Agency/Office)
                             , family=binomial(link="logit"))
...
if(!exists("naics36_Office_term"))
  naics36_Office_term <- glmer(data=large_crisis_smp,
                             b_Term ~  (1 | CrisisProductOrServiceArea/ProdServ) + (1 | Agency:Office)
                             , family=binomial(link="logit"))
get_icc(display=TRUE,naics36_Office_term)

if(!exists("naics36_Office_cbre"))
  naics36_Office_cbre <- glmer(data=large_crisis_smp,
                             b_CBre ~  (1 | CrisisProductOrServiceArea/ProdServ) + (1 | Agency/Office)
                             , family=binomial(link="logit"))
...
if(!exists("ProductOrServiceAreaCode_When_Office_term"))
  naics36_When_Office_term <- glmer(data=large_crisis_smp,
                             b_Term ~  (1 | CrisisProductOrServiceArea/ProdServ) + (1 | StartFY) + (1 | Agency:Office)
                             , family=binomial(link="logit"))
get_icc(display=TRUE,naics36_When_Office_term)
# 
if(!exists("naics36_When_Office_cbre"))
  naics36_When_Office_cbre <- glmer(data=large_crisis_smp,
                             b_CBre ~  (1 | CrisisProductOrServiceArea/ProdServ)+ (1 | StartFY) + (1 | Agency/Office)
                             , family=binomial(link="logit"))
get_icc(display=TRUE,naics36_When_Office_cbre)

```

### ProdServ When Office
```{r psc_When_Office}
if(!exists("psc_When_Office_term"))
  psc_When_Office_term <- glmer(data=large_crisis_smp,
                             b_Term ~  (1 | ProdServ) + (1 | StartFY) + (1 | Agency:Office)
                             , family=binomial(link="logit"))
get_icc(display=TRUE,psc_When_Office_term)

if(!exists("psc_When_Office_cbre"))
  psc_When_Office_cbre <- glmer(data=large_crisis_smp,
                             b_CBre ~  (1 | ProdServ)+ (1 | StartFY) + (1 | Agency:Office)
                             , family=binomial(link="logit"))
...
if(!exists("Place_ssc_psc_cbre"))
  Place_ssc_psc_cbre <- glmer(data=large_crisis_smp,
                       b_CBre ~ (1 | ServCommCons/ProdServ) + 
                         (1 | PlaceCountryISO3)
                       , family=binomial(link="logit"))
get_icc(display=TRUE,Place_ssc_psc_cbre)

if(!exists("Place_cpsa_psc_term"))
  Place_cpsa_psc_term <- glmer(data=large_crisis_smp,
                       b_Term ~ (1 | CrisisProductOrServiceArea/ProdServ)+
                         (1 | PlaceCountryISO3)
                       , family=binomial(link="logit"))
get_icc(display=TRUE,Place_cpsa_psc_term)
...
if(!exists("Vendor_ssc_psc_cbre"))
  Vendor_ssc_psc_cbre <- glmer(data=large_crisis_smp,
                       b_CBre ~ (1 | ServCommCons/ProdServ) + 
                         (1 | VendorCountryISO3)
                       , family=binomial(link="logit"))
get_icc(display=TRUE,Vendor_ssc_psc_cbre)

if(!exists("Vendor_cpsa_psc_term"))
  Vendor_cpsa_psc_term <- glmer(data=large_crisis_smp,
                       b_Term ~ (1 | CrisisProductOrServiceArea/ProdServ)+
                         (1 | VendorCountryISO3)
                       , family=binomial(link="logit"))
get_icc(display=TRUE,Vendor_cpsa_psc_term)
