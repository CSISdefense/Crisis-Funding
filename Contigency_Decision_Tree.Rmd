...
save(full_data,file="LargeDataSets//full_data_after_step_1A.Rdata")
```
...
save(full_data,file="LargeDataSets//full_data_after_step_1B.Rdata")
...
Tree1CdetailIsDefense<-LatticePlotWrapper(
  ...
  coord_flip()+theme(legend.position="bottom")+guide_lengend(nrow=2,byrow=TRUE)
...
defense_data$CrisisFunding[
  defense_data$nationalinterestactioncodeText=="California Wildfires 2008" &
    defense_data$Fiscal.Year<as.Date("2008-01-01") &
    defense_data$CrisisFunding=="Disaster"
]<-"Unlabeled"
...
full_data$CrisisFunding[full_data$CrisisFunding=="Excluded" & full_data$NIAcrisisFunding=="Disaster"]<-"Disaster"
...
save(full_data,file="LargeDataSets//full_data_after_step_2.Rdata")
```
...
save(full_data,file="LargeDataSets//full_data_after_step_3.Rdata")
```
...
save(full_data,file="LargeDataSets//full_data_after_step_4a.Rdata")
```
...
defense_data$pscOCOcrisisPercentRound1<-defense_data$pscOCOcrisisPercent
defense_data$pscOCOcrisisPercentRound1[
  defense_data$pscOCOcrisisPercentRound1<0]<-0
defense_data$pscOCOcrisisPercentRound1<-round(sqrt(defense_data$pscOCOcrisisPercentRound1),2)
...
defense_data$OfficeOCOcrisisPercentRound1<-round(sqrt(defense_data$OfficeOCOcrisisPercentRound1),2)
...
Tree5C<-LatticePlotWrapper(
  ...
  scale_x_continuous(labels = percent_format())+geom_vline(aes(xintercept=0.25))  
Tree5C
ggsave("Output//Tree5C.png", Tree5C+xlab("Square Root of the Share of Office Obligations Going to Official Contingency Contracts"), width=6.5, height= 3, units="in",dpi=300)
sum(defense_data$Obligation.2016[defense_data$CrisisFunding=="OCO"&
      defense_data$Customer=="Defense"& 
