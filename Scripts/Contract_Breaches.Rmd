...
main.text.size<-1
note.text.size<-1.40
if(!exists("fed"))  load(fed,file="data\\clean\\transformed_fed.rdata")
if(!exists("fed_smp"))  load(file="data\\clean\\fed_sample.rdata")
if(!exists("crisis_all"))  load(file="data\\clean\\crisis_population.rdata")
...
NA_stats(fed,"l_Offr")'
* cln_Ceil is the natural log of the initial contract cost ceiling, in then-year dollars. 'r centered_log_description(fed$l_Ceil,"dollars")
NA_stats(fed,"l_Ceil")'
* cln_Days is the natural log of the initial maximum duration of the contract, in days. The variable is centered, by subtracting its mean ('r centered_log_description(fed$l_Days,"days");
NA_stats(fed,"l_Days")' 
...
#Model
CBre03A <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO +cb_Comp+b_Urg, family=binomial(link="logit"))
display(CBre03A)

#Plot the new variable and coefficients
#Add the fitted regression line
discrete_fitted_cbre_model(fed_smp,"cb_Comp") + 
  stat_function(fun = fit_curve, args=list(a=coef(CBre03A)[1],                                                        b=coef(CBre03A)[2]),color="blue")
...
#Add the fitted regression line
discrete_fitted_cbre_model(fed_smp,"cb_Comp") + 
  stat_function(fun = fit_curve, args=list(a=coef(CBre03B)[1],                                                        b=coef(CBre03B)[2]),color="blue")
...
#Model
CBre03C <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO +n_Comp + b_Urg,
                 family=binomial(link="logit"))
display(CBre03C)
...
#   stat_function(fun = fit_curve, args=list(a=coef(CBre03C)[1],
#                                            b=coef(CBre03C)[2]),
#                 color="blue")
...
#Add the fitted regression line
# discrete_fitted_cbre_model(fed_smp,"n_Comp") +
#   stat_function(fun = fit_curve, args=list(a=coef(CBre03D)[1],
#                                            b=coef(CBre03D)[2]),
#                 color="blue")
...
CBre03G_plot<-ggplot(data=fed_smp,
...
discrete_fitted_cbre_model(fed_smp,"cl_Offr") +
  stat_function(fun = fit_curve, args=list(a=coef(CBre03G)[1],
                                           b=coef(CBre03G)[2]),
                             color="blue")

#Plot the fitted values vs actual results
binned_fitted_versus_cbre_residuals(CBre03G)



#Plot residuals versus fitted
residuals_cbre_plot(CBre03G,bins=3)+
...
#Plot the data and fitted curve
CBre03H_plot<-ggplot(data=fed_smp,
       aes(y=j_Term,x=cl_Offr))+geom_jitter(alpha=0.11,height=0)+scale_y_sqrt() +
  labs(title="Fitted Linear Model", caption="Source: FPDS, CSIS Analysis")
...
#Add the fitted regression line
discrete_fitted_cbre_model(fed_smp,"cl_Offr") +
  stat_function(fun = fit_curve, args=list(a=coef(CBre03H)[1],
                                           b=coef(CBre03H)[2]),
                             color="blue")

#Plot the fitted values vs actual results
binned_fitted_versus_cbre_residuals(CBre03H)



#Plot residuals versus fitted
residuals_cbre_plot(CBre03H,bins=3)+
  labs(x="Estimated  Pr (Termination)")
...
CBre_03B <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + cln_Ceil, family=binomial(link="logit"))
display(CBre_03B)
...
#Competition curves
fitted_cbre_model(fed_smp,"cln_Ceil") + stat_function(fun = CBre_03B_curve, 
...
residuals_cbre_plot(CBre_03B,"cln_Ceil")+
...
CBre_03D <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + cln_Ceil + clsqr_Ceil, family=binomial(link="logit"))
display(CBre_03D)
...
#Competition curves
fitted_cbre_model(fed_smp,"cln_Ceil") + stat_function(fun = CBre_03D_curve, 
                             args=list(Comp=0),color="blue")+
...
residuals_cbre_plot(CBre_03D,"cln_Ceil")+
  labs(x="Centered Log(Ceiling)")
...
CBre_03E <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + cln_Ceil + clsqr_Ceil + n_Comp:cln_Ceil, family=binomial(link="logit"))
display(CBre_03E)

CBre_03E_curve<-function(x, Comp){invlogit(cbind(1,Comp,x,x^2,Comp*x) %*%  coef(CBre_03E))}

#Competition Curves
fitted_cbre_model(fed_smp,"cln_Ceil") + stat_function(fun = CBre_03E_curve, 
                             args=list(Comp=0),color="blue")+
...
residuals_cbre_plot(CBre_03E,"cln_Ceil")+
  labs(x="Centered Log(Ceiling)")
```
...
#Model
CBre_03G <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cln_Ceil + clsqr_Ceil + cln_Days + clsqr_Ceil +
                   n_Comp:cln_Ceil , family=binomial(link="logit"))
display(CBre_03G)
...
#Competition Curves
fitted_cbre_model(fed_smp,"cln_Days") + stat_function(fun = CBre_03G_curve, 
...
residuals_cbre_plot(CBre_03G)+
  labs(x="Estimated  Pr (Termination)")
# debug(binned.resids)
# residuals_cbre_plot(CBre_03G,"cln_Days")+
  # labs(x="Centered Log(Ceiling)")
...
CBre_03E <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cln_Ceil + clsqr_Ceil + cln_Days+
                   n_Comp:cln_Ceil  + cln_Ceil:cln_Days, family=binomial(link="logit"))
display(CBre_03E)
...
#Competition Curves
fitted_cbre_model(fed_smp,"cln_Days") + stat_function(fun = CBre_03E_curve, 
                             args=list(Comp=0,Ceil=0),color="blue")+
  stat_function(fun = CBre_03E_curve, 
                             args=list(Comp=2,Ceil=0),color="blue")
...
residuals_cbre_plot(CBre_03E)+
  labs(x="Estimated  Pr (Termination)")
# debug(binned.resids)
# residuals_cbre_plot(CBre_03E,"cln_Days")+
  # labs(x="Centered Log(Ceiling)")
...
CBre_03F <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cln_Ceil + clsqr_Ceil + cln_Days+ clsqr_Days +
                   n_Comp:cln_Ceil  + cln_Ceil:cln_Days, family=binomial(link="logit"))
display(CBre_03F)
...
#Competition curves
fitted_cbre_model(fed_smp,"cln_Ceil") + stat_function(fun = CBre_03F_curve, 
                             args=list(Comp=0),color="blue")+
...
# residuals_cbre_plot(CBre_03F,"cln_Days")+
#   labs(x="Centered Log(Days)")


residuals_cbre_plot(CBre_03F,"cln_Ceil")+
  labs(x="Centered Log(Ceiling)")
...
#Competition curves
fitted_cbre_model(fed_smp,"cln_Ceil") + stat_function(fun = CBre_03G_curve, 
                             args=list(Comp=0),color="blue")+
  stat_function(fun = CBre_03G_curve, 
                             args=list(Comp=2),color="blue")
...
residuals_cbre_plot(CBre_03G,"cln_Ceil")+
  labs(x="Centered Log(Ceiling)")
...
#Create the model
CBre_03H <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cln_Ceil + clsqr_Ceil + cln_Days+ clsqr_Days +
                   n_Comp:cln_Ceil  + cln_Ceil:cln_Days  + n_Comp:cln_Days, family=binomial(link="logit"))
display(CBre_03H)
...
#Competition curves
fitted_cbre_model(fed_smp,"cln_Days") + stat_function(fun = CBre_03H_curve, 
                             args=list(Comp=0,Ceil=0),color="blue")+
...
CBre_03B <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cln_Ceil + clsqr_Ceil + cln_Days+ clsqr_Days +
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cln_Ceil  + cln_Ceil:cln_Days  + n_Comp:cln_Days +
                 SIDV:cln_Days+MIDV:cln_Days+OIDV:cln_Days, family=binomial(link="logit"))
display(CBre_03B)
...
# residuals_cbre_plot(CBre03F,"cln_Days")+
#   labs(x="Centered Log(Days)")
...
CBre_03C <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   ...
                   SIDV + MIDV + FSSGWAC + BPABOA +
                   n_Comp:cln_Ceil  + cln_Ceil:cln_Days  + n_Comp:cln_Days +
                 SIDV:cln_Days+MIDV:cln_Days+OIDV:cln_Days +
...
CBre_03D <- glm (data=fed_smp,
                 ...
                 SIDV:cln_Days+MIDV:cln_Days+OIDV:cln_Days +
                   ...
                   SIDV:cln_Ceil+MIDV:cln_Ceil+OIDV:cln_Ceil, family=binomial(link="logit"))
display(CBre_03D)
...
#Competition curves
discrete_fitted_cbre_model(fed_smp,"cln_Ceil") +
...
CBre_14B <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   ...
                   SIDV:cln_Days+MIDV:cln_Days+OIDV:cln_Days +
                   SIDV:n_Comp+MIDV:n_Comp+OIDV:n_Comp +
                   cln_Days:n_Fixed , family=binomial(link="logit"))
display(CBre_14B)
...
#Create the model
CBre_14D <- glm (data=fed_smp,
                 b_CBre ~ ARRA + Dis + OCO + n_Comp + 
                   cln_Ceil + cln_Days+ clsqr_Ceil + clsqr_Days +
                   SIDV + MIDV + FSSGWAC + BPABOA + 
                   n_Comp:cln_Ceil  + cln_Ceil:cln_Days  + n_Comp:cln_Days +
