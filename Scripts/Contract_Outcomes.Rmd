...
First we load the data. The dataset used is a U.S. federal Contracting dataset derived from   FPDS. The crisis sample is created by including the entirity of the OCO, ARRA, and Disaster datasets.
...
load(file="..//data//clean//fed_sample.Rdata")
load(file="..//data//clean//crisis_population.Rdata")
```
...
corrdisp<-Hmisc::rcorr(as.matrix(cbind(subset(crisis_smp, select=c(
  b_UCA,c_OffCri,OffIntl,cln_Ceil,
  cl_Days,n_Fixed,b_Intl)),
  dummies::dummy(crisis_smp$Crisis),
  dummies::dummy(crisis_smp$NoCompOffr))))
...
For Ceiling breaches, the new relationship is significant but also throws off fitted vs. actual values and increases residuals. For terminations, the coefficients are greater and cln_Ceiling gains a larger coefficient. Dropping for both.
...
This causes cln_Ceil VIF to exceed 2, and thus it must be removed.

#### 05H: Threshold and ceiling interaction

```{r Model05H}
summary_continuous_plot(large_crisis_smp,"cln_Ceil","thresh")
...
CBre_06A <- glm (data=large_crisis_smp,
                 b_CBre ~ Crisis +NoCompOffr +
                   ...
                   cln_Ceil+
                   cln_Days+
                   cln_Ceil+Crisis:NoCompOffr+b_UCA:Crisis+cln_Ceil:NoCompOffr, 
                 family=binomial(link="logit"))
...
summary_double_continuous(large_crisis_smp,"cln_Days","cln_Ceil")
...
Unfortunately, VIF knocks out this interaction as in both cases cln_Days goes above 2.0.
...
This addition pushes the VIF for cln_Ceil above 2 for terminations. Crisis:cln_Ceil will be removed. After which, the VIF for NoCompOffr exceeded 2. The study team chose to remove b_NoCompOffrTRUE:cln_Days as when examining the descriptive statistics, there was a more noticable difference in pattern for that set of variables and better alignment between the Term and Ceil models is an additional feature.
...
CBre_08B <- glm (data=large_crisis_smp,
                 b_CBre ~ Crisis +NoCompOffr +
                   ...
                   Veh+
                   n_Fixed +
                  cln_Ceil+Crisis:NoCompOffr+b_UCA:Crisis+cln_Ceil:NoCompOffr+cln_Days:NoCompOffr+
...
```{r Model08H}
  ...
  CBre_08H <- glm (data=large_crisis_smp,
                   b_CBre ~ Crisis +NoCompOffr +
                     ...
                     Pricing +
                    cln_Ceil+Crisis:NoCompOffr+
                     b_UCA:Crisis+cln_Ceil:NoCompOffr+
                     cln_Days:NoCompOffr,
                   ...
                   b_Term ~ Crisis +NoCompOffr+
                    ...
                    cln_Ceil+
                     ...
                     # Is.Defense +
                                        # Crisis:NoCompOffr+
                     # b_UCA:Crisis+
                     # cln_Ceil:NoCompOffr+ Don't believe this is compaible with Crisis:NoCompOffr
                     # b_UCA:Crisis+
                     # cln_Days:Veh+ 
                     # cln_Ceil:Veh+
                     # Crisis:Is.Defense+
                   ...
                   b_Term ~ Crisis +NoCompOffr2+
                    ...
                    cln_Ceil+
                     ...
                     # Is.Defense +
                                        # Crisis:NoCompOffr2+
                     # b_UCA:Crisis+
                     # cln_Ceil:NoCompOffr2+ Don't believe this is compaible with Crisis:NoCompOffr2
                     # b_UCA:Crisis+
                     # cln_Days:Veh+ 
                     # cln_Ceil:Veh+
                     # Crisis:Is.Defense+
...
if(!exists("CBre_20B"))
  CBre_20B <- glmer (data=large_crisis_smp,
                   b_CBre ~ Crisis +NoComp +
                     ...
                     PricingFee +
                     # Is.Defense +
                    # Crisis:NoComp+b_UCA:Crisis+cln_Ceil:NoComp+cln_Days:NoComp+
                     # cln_Days:Veh+ 
                     # cln_Ceil:Veh+
                   # NoComp:Veh+
                     # Reach:Crisis+
  ...
  CBre_20C <- glmer (data=large_crisis_smp,
                   b_CBre ~ Crisis +NoComp +
                    ...
                    Crisis:NoComp+b_UCA:Crisis+
                     #+cln_Ceil:NoComp+cln_Days:NoComp+
                     # cln_Days:Veh+ 
                     # cln_Ceil:Veh+
                   # NoComp:Veh+
                     # Reach:Crisis+
...
summary(CBre_21E)
  ...
  save(CBre_21E,  file="../Output//Model21E_CBre_2020-7-31.RData")
...
if(!exists("CBre_22G"))
  ...
  save(Term_22G, file="../Output//Model22G_Term.RData")
# Plot the model and Vehicle 
...
if(!exists("CBre_22H")){
  ...
  save(CBre_22H, file="..//Output//Model22H_2020-07-31.RData")
...
save(CBre_22H,CBre_22H.all, file="../Output//Model22H_2020-07-31")
