...
MilPersXLS <- loadWorkbook(file.path("Data","FY2017","m1.xlsx"))
...
MilPersData$SourceFiscalYear<-2017
...
```{r OnM}
# Load workbook o1
OnMXLS <- loadWorkbook(file.path("Data","FY2017","o1.xlsx"))
...
```{r RevolvingFunds}
# Load workbook RF
RFXLS <- loadWorkbook(file.path("Data","FY2017","rf1.xlsx"))
...
colnames(RFdata)
RFdata$SourceFiscalYear<-2017
RFdata<-ComptrollerMelt(RFdata)
RFdata<-ComptrollerVariableRename(RFdata)
write.csv(RFdata,"RF.csv")

# was it a mistake to have this in here before the {R OCO} block? - LCL
...
ProcXLS <- loadWorkbook(file.path("Data","FY2017","p1.xlsx"))
...
colnames(ProcData)
ProcData$SourceFiscalYear<-2017
...
write.csv(ProcData,paste("Data\\","P1_2017_Consolidated.csv",sep=""), 
          row.names=FALSE,
          na="")
str(ProcData)
...
RDTEXLS <- loadWorkbook(file.path("Data","FY2017","r1.xlsx"))
...
colnames(RDTEdata)
RDTEdata$SourceFiscalYear<-2017
