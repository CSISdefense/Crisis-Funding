...
# defines a list of CrisisFunding for use in the customer selectInput.
# defining it seperately isn't necessary and only saves typing it twice in the
# selectInput command in the ui section
CrisisFunding <- c("Disaster", "OCO", "ARRA", 
               "Residual")

# same thing for list of platform Place
Place <- c("Afghanistan", "Iraq",
                "Regional Support", "Rest of World",
                "Domestic")
...
ui <- fluidPage(
    fluidRow(
        ...
        column(3, align = 'center',
               ...
               # Settings for Funding select
               selectInput("CrisisFunding","CrisisFunding", CrisisFunding,
                           multiple = TRUE,
                           selectize = FALSE,
                           selected = CrisisFunding,
                           width = '100%',
                              size = 6),
                  br(),
                  
                  # Settings for Place select
...
dataset <- reactive({
    ...
    shown <- filter(FullData,
      ...
      # strung together 
      shown <- filter(shown, Place %in% input$Place &
                             Area %in% input$Area &
                             CrisisFunding %in% input$CrisisFunding)
...
#        writedata <- FullData
#        writedata$FY <- as.numeric(as.character(writedata$FY)) + 2000
#        writedata <- select(writedata, FY, VendorSize, Customer, Area,
#                            Place, Amount)
#        write.csv(writedata, file)
  #}
#)
