...
shinyServer(function(input, output, session) {
  ...
  FullData <- read.csv(ZipFile,
                       na.strings="NULL")
  rm(ZipFile)
  # FullData$Fiscal.Year<-as.integer(as.character(FullData$Fiscal.Year))
  # FullData<-subset(FullData,!is.null(SubCustomer) & SubCustomer!="NULL")
  Path<-"K:\\2007-01 PROFESSIONAL SERVICES\\R scripts and data\\"
  FullData<-apply_lookups(Path,FullData)
  
  original_data<-subset(FullData, year(Fiscal.Year)>=2000)
  original_data$Fiscal.Year<-year(original_data$Fiscal.Year)
  # set correct data types
  original_data %<>%
    # select(-ContractingCustomer) %>%
    mutate(SubCustomer = factor(ContractingSubCustomer)) %>%
    mutate(ProductOrServiceArea = factor(ProductOrServiceArea)) %>%
    mutate(CrisisFunding = factor(CrisisFunding)) %>%
    mutate(Vendor.Size = factor(Vendor.Size)) %>%
    # mutate(nationalinterestactioncodeText = factor(nationalinterestactioncodeText)) %>%
    # mutate(ClassifyNumberOfOffers = factor(ClassifyNumberOfOffers)) %>%
    mutate(Simple = factor(SimpleArea))
