...
for(sfy in SourceFolderList){
  ...
  OnMXLS <- XLConnect::loadWorkbook(file.path("Data",sfy,"o1.xlsx"))
...
for(sfy in SourceFolderList){
  ...
  RFXLS <- loadWorkbook(file.path("Data",sfy,"rf1.xls"))
  ...
  write.csv(RFdata,file.path("Data",sfy,"Output",paste("OnM(",NumericFiscalYear,").csv",sep="")))
