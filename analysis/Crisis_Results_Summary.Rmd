...
library(reshape2)
library(lubridate)
library(showtext) #Not sure I actually need this.
library(extrafont)
library(extrafontdb)
library(tidyverse)
...
load(file.path("..","data","clean","fed_transformed.Rdata"))
load(file="..//Data//semi_clean//full_data_after_step_6.Rdata")
...
if(! "PlaceIntlPercent" %in% colnames(full_data) & !"CrisisPercent"  %in% colnames(full_data)){
  colnames(full_data)[colnames(full_data)=="ContractingOfficeID"]<-"ContractingOfficeCode"
...
if(! "OffPlace" %in% colnames(full_data)){
  full_data$OffPlace<-Hmisc::cut2(full_data$PlaceIntlPercent,c(0.01,0.50))
  levels(full_data$OffPlace) <-
...
summary_data<-summary_data %>% group_by(DecisionTreeDisplay,OffPlace) %>%
                    dplyr::mutate(
                    pDatasetOffPlaceObligation=Obligation.2017/sum(Obligation.2017,na.rm=TRUE))




summary_data$IsAnyCrisis<-summary_data$CrisisDisplay
levels(summary_data$IsAnyCrisis)<-list("All Other"="All Other",
                                       "Any Crisis"=c("OCO","ARRA","Disaster"))


summary_data$Competition.multisum<-summary_data$NoCompOffr
levels(summary_data$Competition.multisum)<-
      list("Other No"  =c("Other No"  ),
           "Urgency" = "Urgency",
           "1 Offer"="1 Offer",
           "2+ Offers"= c("2-4 Offers", "5+ Offers"),
                                   "Unlabeled"=c("Unlabeled"))

summary(summary_data$NoCompOffr)
...
summary_data$SelectCountry<-as.character(summary_data$OMBtheater)
#Countries, other than the U.S. with at least $5 billion in cumulative OCO
...
summary_data$SelectCountry[summary_data$PlaceCountryText %in% country_list]<-
  as.character(summary_data$PlaceCountryText[
    summary_data$PlaceCountryText %in% country_list])
summary_data$SelectCountry<-factor(summary_data$SelectCountry,
                                   levels=c("AFGHANISTAN",
                "IRAQ",
                "KUWAIT",
                "UNITED ARAB EMIRATES",
                "BAHRAIN",
...
summary_data$Competition.sum<-summary_data$NoCompOffr
levels(summary_data$Competition.sum)<-(
  list(
  "No Comp."=c("Other No","Urgency"),
  "Competition" = c("1 Offer","2-4 Offers","5+ Offers"),
  "Unlabeled"="Unlabeled" )
)
...
save(labels_and_colors,column_key,complete_summary_data,file="../scripts/Shiny/Crisis_Funding.RData")
```
...
chapter4topline<-LatticePlotWrapper_csis360(
  ...
  scale_x_date("Fiscal Year",labels = scales::date_format("'%y"), date_breaks="3 years")+
  geom_vline(xintercept=as.numeric(as.Date("2006-07-01")))
)



disaster_summary<-full_data %>%  filter(CrisisDisplay=="Disaster") %>%
  group_by(CrisisDisplay,nationalinterestactioncodeText,Fiscal.Year) %>%
  dplyr::summarise(obligations=sum(Obligation.2017)) %>% group_by(Fiscal.Year) %>%
  dplyr::mutate(annual_share=round(obligations/sum(obligations),3)) %>% group_by(nationalinterestactioncodeText) %>%
  dplyr::mutate(disaster_share=round(obligations/sum(obligations),3)) 

disaster_summary<-disaster_summary[order(disaster_summary$Fiscal.Year),]
write.csv(disaster_summary,file="..//Output//disaster_summary.csv")
chapter4topline$data %>%  group_by(primary,x.variable) %>%
  dplyr::summarise(obligations=sum(y.variable))


ggsave600dpi("..//Output//Figure14_topline.png", chapter4topline, width=6.5, height= 4, units="in")

ggsave("..//Output//Figure14_topline.eps", device=cairo_pdf, chapter4topline, width=6.5, height= 4, units="in")
write.csv(chapter4topline$data,file="..//Output//Figure14_topline.csv")
```
...
(
CompOverall<-build_plot(
  data=subset(summary_data,
                     DecisionTreeDisplay!="Not in Sample"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
  x_var="DecisionTreeDisplay", #x_var
  y_var="pDatasetObligation", #VAR.y.variable
 ...
 theme(axis.text.x = element_text(angle=90))+
  ...
  expand_limits(y = c(0,0.2))+
        geom_text(aes(label=ifelse(pDatasetObligation<=0.015,round(pDatasetObligation,3)*100,round(pDatasetObligation,2)*100),
                      x=DecisionTreeDisplay,
...
CompOverall$data %>%  group_by(primary,x.variable) %>%
  dplyr::summarise(obligations=sum(y.variable))


ggsave600dpi("..//Output//Figure15_CompOverall.png", CompOverall, 
             width=8, height= 3, units="in",size=11
             )




ggsave("..//Output//Figure15_CompOverall.eps", device=cairo_pdf, CompOverall, 
...
comp_summary<-CompAnnual$data %>%  group_by(dFYear,DecisionTreeDisplay,NoCompOffr) %>%
  dplyr::summarise(obligations=sum(Obligation.2017))%>%  
  group_by(dFYear,DecisionTreeDisplay) %>%
  dplyr::mutate(share=obligations/sum(obligations))
write.csv(comp_summary,file="..//Output/compsummary.csv")


(
CompAnnualShare<-build_plot(
  data=summary_data %>% filter(DecisionTreeDisplay %in% c("Disaster","ARRA","OCO")),
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
...
ggsave("..//Output//Figure16_CompAnnual.png", CompAnnual, 
             width=2.75, height= 4.5, units="in"
             )


ggsave("..//Output//Figure16_CompAnnual.eps", device=cairo_pdf, CompAnnual, 
             width=2.75, height= 4.5, units="in"
             )
...
UCAoverall<- build_plot(
 ...
 theme(axis.text.x = element_text(angle=90))+
    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
      facet_grid(.~isUCA ,scales="free_x", space="free_x"
             )+labs(title=NULL,
  # xlab="Fiscal Year",
  # ylab="Percent of Obligations",
  color="Competition")+
...
ggsave600dpi("..//Output//UCAoverall.png", UCAoverall, width=8, height= 2.75, units="in",size=11)
...
(
OMBtheater<-LatticePlotWrapper_csis360(
  VAR.color.legend.label="Customer",
  VAR.main.label=NULL,
  VAR.X.label="Fiscal Year",
  VAR.Y.label="Obligations (2017 $ Billions)",
  VAR.Coloration=labels_and_colors,
  VAR.long.DF=subset(complete_summary_data,!is.na(International) & International!="Unlabeled" &
                       Is.Defense!="Unlabeled"),
  # NA, #VAR.ncol
  VAR.x.variable="dFYear", #VAR.x.variable
  VAR.y.variable="Obligation.2017", #VAR.y.variable
  VAR.y.series="OffPlace", #VAR.y.series
  ...
  DataLabels=FALSE
             ...
             )+theme(legend.position = 'bottom')+get_caption()+#+scale_fill_discrete(guide=guide_legend(ncol=2))
  scale_x_date("Fiscal Year",labels = date_format("'%y"), date_breaks="3 years")+
  geom_vline(xintercept=as.numeric(as.Date("2006-07-01")))+labs(caption="Note: Excludes Unlabeled Place of Performance\nSource: FPDS; CSIS analysis")
)



OMBtheater$data %>%  group_by(primary,x.variable) %>%
  dplyr::summarise(obligations=sum(y.variable))

OMBtheater$data %>%  group_by(primary,category) %>%
  dplyr::summarise(obligations=sum(y.variable)) %>%
  group_by(primary)%>%dplyr::mutate(offplace_share=round(obligations/sum(obligations),3))

# ALl of the defense graphed data, percent breakdown.
OMBtheater$data %>% filter(secondary=="Defense") %>% group_by(primary,category,x.variable) %>%
  dplyr::summarise(obligations=sum(y.variable)) %>%
  group_by(x.variable,primary)%>%dplyr::mutate(offplace_share=round(obligations/sum(obligations),3))


OMBtheater$data %>%  group_by(primary,secondary,category) %>%
  dplyr::summarise(obligations=sum(y.variable,secondary)) %>%
  group_by(primary,secondary)%>%dplyr::mutate(offplace_share=round(obligations/sum(obligations),3)) %>%
  filter(secondary=="Civilian")


OMBtheater$data %>% filter(secondary=="Defense") %>% group_by(primary,secondary,category) %>%
  dplyr::summarise(obligations=sum(y.variable)) %>%
  group_by(primary,secondary)%>%dplyr::mutate(offplace_share=round(obligations/sum(obligations),3))
  


complete_summary_data %>% filter(Is.Defense ==FALSE & OMBtheater!="Domestic") %>%
  group_by(Is.Defense,CrisisDisplay,OffPlace)%>%
  dplyr::summarise(obligations=sum(Obligation.2017)) %>%
  group_by(Is.Defense,CrisisDisplay)%>%dplyr::mutate(offplace_share=round(obligations/sum(obligations),3))
  
  
ggsave("..//Output//figure19_offplace.png", OMBtheater, width=6.5, height= 4, units="in")

ggsave("..//Output//figure19_offplace.eps", device=cairo_pdf, OMBtheater, width=6.5, height= 4, units="in")


(
ReachbackCrisisHistory<-LatticePlotWrapper_csis360(
  VAR.color.legend.label="Customer",
  VAR.main.label=NULL,
  VAR.X.label="Fiscal Year",
  VAR.Y.label="Obligations (2017 $ Billions)",
  ...
  # VAR.facet.secondary="OffPlace", # VAR.facet.secondary=NA
  # ,MovingAverage=0
  # ,MovingSides=1
  # ,DataLabels=NA
  #                       ,VAR.override.coloration=NA
  DataLabels=FALSE
)+theme(legend.position = 'bottom')+get_caption()+
  scale_x_date("Fiscal Year",labels = date_format("'%y"), date_breaks="3 years")+
  geom_vline(xintercept=as.numeric(as.Date("2006-07-01")))+labs(caption="Note: Excludes Unlabeled Place of Performance\nSource: FPDS; CSIS analysis")
...
#   VAR.y.variable="pDatasetOffPlaceObligation", #VAR.y.variable
...
ggsave("..//Output//ReachbackComp.png", ReachbackComp, width=4.5, height= 8, units="in")
...
##### Contracting Office
```{r Koff}
full_data$ContractingOfficeCode
office_data<-summary_data %>%
  dplyr::group_by(CrisisDisplay,
                  # SupplyServiceERS,     #Should switch this to CrisisProdServ
                  # Reachback,            #Swap in OffPlace
                  CrisisPercent,
                  DecisionTreeDisplay,
...
(
  ggplot(
    data=office_data %>% filter(CrisisDisplay=="OCO"),
    aes(x=CrisisPercent,
        y=Obligation.2017,
        color=OffPlace)
...
(
#+facet_wrap( "CrisisDisplay" )
CrisisPercent<-build_plot(
  data=subset(summary_data,
                     DecisionTreeDisplay!="Not in Sample"),
...
str(office_data$CrisisPercent)

  ggplot(data=office_data,
         aes(x=CrisisPercent,
             weight=Obligation.2017,
             fill=OffPlace))+
    geom_histogram(bins=100)+facet_grid(DecisionTreeDisplay ~., scales="free_y")
  

```


##### Country : Office Place
```{r Country}
...
(
CountryOffPlace<-LatticePlotWrapper_csis360(
  VAR.color.legend.label=NULL,
  VAR.main.label=NULL,
  VAR.X.label="Fiscal Year",
  VAR.Y.label="Obligations (2017 $ Billions, Variable Scale)",
  VAR.Coloration=labels_and_colors,
  VAR.long.DF=complete_summary_data%>% filter(SelectCountry != "Domestic")  ,
  # NA, #VAR.ncol
  VAR.x.variable="dFYear", #VAR.x.variable
  VAR.y.variable="Obligation.2017", #VAR.y.variable
  VAR.y.series="OffPlace", #VAR.y.series
  VAR.facet.primary="SelectCountry", #VAR.facet.primary
  VAR.facet.secondary= "IsAnyCrisis", # VAR.facet.secondary=NA
  # ,MovingAverage=0
  # ,MovingSides=1
  DataLabels=FALSE
  #                       ,VAR.override.coloration=NA
    ...
    guides(fill = guide_legend(ncol = 2))+
    scale_x_date("Fiscal Year",labels = date_format("'%y"), date_breaks="4 years")
)

ggsave("..//Output//Figure20_CountryOffPlace.png", CountryOffPlace, width=4.5, height= 8, units="in")
2413719774 -1262213240.3	
1010956482.9	-1986585571.2	
summary(CountryOffPlace$data$category)

CountryOffPlace$data %>%  group_by(primary,category,x.variable) %>%
  dplyr::summarise(obligations=sum(y.variable)) %>%
  group_by(primary,x.variable)%>%dplyr::mutate(country_share=round(obligations/sum(obligations),3))%>% filter(category=="Mixed")
...
ggsave("..//Output//Customer.eps", device=cairo_pdf, Customer, width=4, height= 5, units="in")
```
...
ggsave("..//Output//Duration.eps", device=cairo_pdf, Duration, width=4, height= 5, units="in")
...
DurationComp<-  LatticePlotWrapper_csis360(
  ...
  VAR.y.series="NoCompOffr", #VAR.y.series
  VAR.facet.primary="DecisionTreeDisplay", #VAR.facet.primary
  VAR.facet.secondary="Dur.Simple" # VAR.facet.secondary=NA
  # ,MovingAverage=0
  # ,MovingSides=1
   ,DataLabels=FALSE
  #                       ,VAR.override.coloration=NA
  ...
  theme(strip.text.y = element_text(angle=0))+
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(primary ~ secondary,scales="free_y"#, space="free_y"
             )
    )
...
ggsave("..//Output//ProductService.eps", device=cairo_pdf, UCAoverall, width=4, height= 5, units="in")
```
...
load(file.path("..","data","clean","fed_sample.Rdata"))
load(file.path("..","data","clean","crisis_population.rdta"))
...
stargazer::stargazer(Term_21D,type="text",#Term_20A,#Term_20C
                        digits=2)

summary_residual_compare(Term_21D,bins=50)

  # CBre_22H.all<-allFit_save(CBre_22H,filename="..//Output/CBre_22H_allfit.rdata")
```


### Federal-Wide 21E: CBre
```{r Model21E}


load(file.path("..","..//Output","Model21E_CBre.rdata"))

  glmer_examine(CBre_21E.restart)

  
# Plot the model and Vehicle 
stargazer::stargazer(CBre_21E.restart,type="text",#Term_20A,#Term_20C
                        digits=2)

summary_residual_compare(CBre_21E.restart,bins=50)


  # CBre_21E.all<-allFit_save(CBre_21E.restart,filename="..//Output/CBre_21E_allfit.rdata")
```


### Crisis-Only 22G Term
```{r Model22G}
# summary(large_crisis_smp$ServCommCons)
# summary(large_crisis_smp$CPSA)

load(file.path("..","..//Output","Model22G.RData"))
  ...
  # Term_22G_ss_cons <- summary(Term_22G.all) 
  # 
...
# Plot the model and Vehicle 
stargazer::stargazer(Term_22G,type="text",#Term_20A,#Term_20C
                        digits=2)

summary_residual_compare(Term_22G,bins=50)
```
...
### Crisis-Only 22H CBre
```{r Model22H}
# summary(large_crisis_smp$ServCommCons)
# summary(large_crisis_smp$CPSA)

load(file.path("..","..//Output","Model22H.RData"))
...
summary(fed_1m[,colnames(fed_1m) %in% 
  ...
  load("..//Output//CBre_22H_allfit.rdata")

# Plot the model and Vehicle 
stargazer::stargazer(CBre_22H,res[1],res[2],res[3],#res[4],
                     res[5],#res[6],res[7],
...
summary_residual_compare(CBre_22H,bins=50)
...
coef(Term_22G)$PlaceCountryISO3[1]
Country_Intercept <- as.data.frame(coef(Term_22G)$PlaceCountryISO3[1])
...
gridExtra::grid.arrange(dotplot(ranef(CBre_21E.restart, condVar = T), strip = T, scales=list(relation='free'))$Agency,
             dotplot(ranef(CBre_22H, condVar = T), strip = T, scales=list(relation='free'))$Agency,
             nrow=1)
...
stargazer::stargazer(CBre_21E.restart,CBre_22H,Term_21D,Term_22G,type="text",
                       digits=2)


summary_residual_compare(CBre_21E.restart,CBre_22H,Term_21D,Term_22G,bins=50)


texreg::htmlreg(list(CBre_21E.restart,CBre_22H,Term_21D,Term_22G),file=file.path("..","..//Output","Both_Model.html"),
                single.row = TRUE,
...
#Note, must have datasets loaded.
or_cbre_overall<-odds_ratio(CBre_21E.restart,"CBre_21E.restart","Federal","Ceiling Breaches",
                            walds = TRUE,rename_list=all_coef_list)
or_cbre_crisis<-odds_ratio(CBre_22H,"CBre_22H","Crisis","Ceiling Breaches",
                           walds = TRUE,rename_list=all_coef_list)
comp.or<-rbind(or_cbre_overall,or_cbre_crisis)


write.csv(get_study_variables_odds_ratio(comp.or,study="crisis"),file=
            file.path("..","..//Output","ceiling_breach_study_odds_ratio.csv"),row.names=FALSE)



or_term_overall<-odds_ratio(Term_21D,"Term_21D","Federal","Terminations",
                            walds = TRUE,rename_list=all_coef_list)
or_term_crisis<-odds_ratio(Term_22G,"Term_22G","Crisis","Terminations",
                           walds = TRUE,rename_list=all_coef_list)
term.or<-rbind(or_term_overall,or_term_crisis)

write.csv(get_study_variables_odds_ratio(term.or,study="crisis"),file=
            file.path("..","..//Output","terminations_study_odds_ratio.csv"),row.names=FALSE)
...
(
  ceiling_breach_residual<-gridExtra::grid.arrange(
    binned_fitted_versus_residuals(CBre_21E.restart,bins=50)+
      labs(title="Overall Ceiling Breach Binned Actuals",caption=NULL,
           x="Estimated Pr(Breach)",y="Actual Pr(Breach)"),
    residuals_binned(CBre_21E.restart, bins=50)+labs(title="Overall Ceiling Breach Binned Residuals",
                                              x="Estimated Pr(Breach)",y="Residuals"),
    binned_fitted_versus_residuals(CBre_22H,bins=50)+
      labs(title="Crisis Ceiling Breach Binned Actuals",caption=NULL,
           x="Estimated Pr(Breach)",y="Actual Pr(Breach)"),
    residuals_binned(CBre_22H, bins=50)+labs(title="Crisis Ceiling Breach Binned Residuals",
                                              x="Estimated Pr(Breach)",y="Residuals")
  )
)
ggsave(ceiling_breach_residual,file=file.path("..","..//Output","cbre_residual.png"),width=6.5*1.2, height=8*1.2)



(
  terminations_residual<-gridExtra::grid.arrange(
    binned_fitted_versus_residuals(Term_21D,bins=50)+
      labs(title="Overall Terminations Binned Actuals",caption=NULL,
           x="Estimated Pr(Terminations)",y="Actual Pr(Terminations)"),
    residuals_binned(Term_21D, bins=50)+labs(title="Overall Terminations Binned Residuals"),
    binned_fitted_versus_residuals(Term_22G,bins=50)+
      labs(title="Crisis Terminations Binned Actuals",caption=NULL,
           x="Estimated Pr(Terminations)",y="Actual Pr(Terminations)"),
    residuals_binned(Term_22G, bins=50)+labs(title="Crisis Terminations Binned Residuals"),
    ncol=2
    )
)
  ggsave(terminations_residual,file=file.path("..","..//Output","term_residual.png"),width=6.5*1.2, height=8*1.2)
