...
fed_full$Offr <- cut2(fed_full$UnmodifiedNumberOfOffersReceived,cuts=c(1,2,3,5))



if (all(levels(fed_full$Offr)==c("  1","  2","[  3,  5)","[  5,999]"))){
  fed_full$Offr<-factor(fed_full$Offr, 
...
fed_full$Offr[is.na(fed_full$Offr)&
...
#Produce EffComop factor

#Competition and Offer  
fed_full$EffComp<-as.character(fed_full$Comp)

fed_full$EffComp[fed_full$Comp=="Comp." & !is.na(fed_full$Comp)]<-
  ifelse(fed_full$UnmodifiedNumberOfOffersReceived[fed_full$Comp=="Comp." & 
                                                     !is.na(fed_full$Comp)]>1,"2+ Offers","1 Offer")
